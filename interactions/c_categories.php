<?php
    ...
    function c_get_crm_categories() {
        ...
        try {
            ...
            // // logger('INFO--categories/c_get_crm_categories-- URL:'.$url);
            // $request = curl_init($url); 
            // curl_setopt($request, CURLOPT_RETURNTRANSFER, true);
            // curl_setopt($request, CURLOPT_HEADER, 0);
            // logger('INFO--categories/c_get_crm_categories-- Connection possibility requesting...');
            ...
            //     if($array->is_sync_allowed==true){
                    ...
                    if($httpcode==200){
                        ...
                        if($array->queryResultCode==1){
                            ...
                            $all_crm_woo_ids = $array->allProductCategoriesWooIds;
                            ...
                            foreach ($array->productCategories as $crm_category ) {
                                ...
                                array_push($all_crm_woo_ids_to_sync, $crm_category->woo_id); 
                            ...
                            print_r ($all_woo_ids);
                            echo('<br>');
                            echo('<b>$all_crm_woo_ids_to_sync: </b><br>');
                            print_r ($all_crm_woo_ids_to_sync);
                            echo('<br>');
                            ...
                            foreach ($all_woo_ids as $curr_woo_id ) {
                                if (!in_array($curr_woo_id, $all_crm_woo_ids) && $curr_woo_id != $uncategorized_id){
                                    if(!$wasDelLog){$wasDelLog=true; logger('INFO--categories/c_get_crm_categories-- Deleting categories that was deleted in CRM:');}
                            ...
                            // in a case if parent crm id not in a ids (it can produce infinity cycle "while...")
                            foreach ($array->productCategories as $crm_category ) {
                                if($crm_category->parent_crm_id != 0 && !in_array($crm_category->parent_crm_id, $all_crm_ids))
                                    $crm_category->parent_crm_id = 0;                                
                            }

                            $cycle_counter=0; // needs for analyze of working in logs
                            //logger("all_crm_ids - ".count($all_crm_ids));
                            ...
                            logger('INFO--categories/c_get_crm_categories-- Start cycle #'.$cycle_counter);    
                                ...
                                { 
                                    ...
                                    //logger("synchronized_crm_ids - ".count($synchronized_crm_ids));
                                    ...
                                    if(($category->parent_crm_id == 0 || in_array($category->parent_crm_id, $synchronized_crm_ids)) && !in_array($category->crm_id, $synchronized_crm_ids))
                                    {
                                        ...
                                        // And firsteval before update I need to check that WooCommerce has the category with this woo_id:
                                            ...
                                            if (in_array($category->woo_id, $all_woo_ids)) {
                                                ...
                                                array_push($ids_pairs_to_post_crm, (object) ['id'=>$category->woo_id, 'crm_id'=>$category->crm_id]);
                                                array_push($ids_pairs_to_synchronize, (object) ['id'=>$operation_result->id, 'crm_id'=>$category->crm_id]);
            ...
            //         logger('INFO--categories/c_get_crm_categories-- Connection possibility rejected. Reason: '.$array->reason);  
            //     }


            // } else {
            //     echo '<b>Server error with response code = '.$httpcode.' Connection possibility request failed!</b><br>';
            //     logger ('ERROR--categories/c_get_crm_categories-- Server error with response code = '.$httpcode.', Response = '.$response.' Connection possibility request failed!, Received data = '.$data);
            // }
