<?php
    ...
    function c_get_crm_tax_rates() {
        ...
        try {
            ...
            logger('INFO--taxes/c_get_crm_tax_rates-- Connection possibility requesting...');
            ...
            if($httpcode==200){
                ...
                if($array->is_sync_allowed==true){
                    ...
                    $url = get_option( 'API_address' ).'/syncTaxesToStore?key='.get_option( 'secret_key' );
                    $request = curl_init($url); 
                    curl_setopt($request, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($request, CURLOPT_HEADER, 0);
                    ...
                    if(count($ids_pairs) >0 ){
                        ...
                        $url = get_option( 'API_address' ).'/syncTaxesIds';
                        $curl = curl_init($url);
                    ...
                    logger('INFO--taxes/c_get_crm_tax_rates-- Connection possibility rejected. Reason: '.$array->reason);  
                }


            } else {
                echo '<b>Server error with response code = '.$httpcode.' Connection possibility request failed!</b><br>';
                logger ('ERROR--taxes/c_get_crm_tax_rates-- Server error with response code = '.$httpcode.', Response = '.$response.' Connection possibility request failed!, Received data = '.$data);
            }
    ...
    //         echo '<pre><code>' . print_r($e->getRequest(), true) . '</code><pre>'; // Last request data.
    //         echo '<pre><code>' . print_r($e->getResponse(), true) . '</code><pre>'; // Last response data.
    //         logger ('ERROR--taxes/c_get_crm_tax_rates-- The response: '.print_r($e->getResponse()));
    //     } catch (Exception $e) {
    //         echo 'Exception: ',  $e->getMessage(), "\n";
    //         logger ('ERROR--taxes/c_get_crm_tax_rates-- The response: '.$e->getMessage());
    //     }
